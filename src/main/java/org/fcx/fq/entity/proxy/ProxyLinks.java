package org.fcx.fq.entity.proxy;
    ...
    public void loadLinks () {
        try {
            if(!proxyLinksFile.exists()) {
                throw new MyToolException("加载代理链接出错："+proxyLinksFile.getFile().getAbsolutePath()+"不存在");
            }
            linksMap = objectMapper.readValue(proxyLinksFile.getFile(),HashMap.class);
            log.debug("{}",linksMap);
        } catch (IOException e) {
            log.error("init proxy links err",e);
            throw new MyToolException("init proxy links err",e);
        }
    }
